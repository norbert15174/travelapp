...
function connect() {
    client = Stomp.client('ws://localhost:8020/chat');
    color = getRandomColor();
    client.connect({}, function (frame) {
        client.subscribe("/topic/5", function(message){
...
function sendMessage() {
    ...
    client.send("/app/chat/5", {}, JSON.stringify({'value': messageToSend, 'user': user, 'userColor': color}) );
