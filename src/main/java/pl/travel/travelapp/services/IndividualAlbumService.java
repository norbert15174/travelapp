...
public class IndividualAlbumService implements IndividualAlbumInterface, CoordinateInterface, SharedAlbumInterface {
    ...
    public ResponseEntity deleteAlbum(Principal principal , long id) {
        ...
        if ( individualAlbumRepository.findIndividualAlbumById(id , personalQueryService.getPersonalInformation(principal.getName()).getId()).isPresent() ) {
    ...
    public ResponseEntity <List <AlbumDTO>> getAvailableAlbums(Principal principal) {
        ...
        return new ResponseEntity(sharedAlbum.stream().map(shared -> new AlbumDTO().build(shared.getIndividualAlbum())).sorted().collect(Collectors.toList()) , HttpStatus.OK);
    }
