...
public interface FriendsRequestRepository extends JpaRepository <FriendsRequest, Long> {
    @Query("select f from FriendsRequest f left join fetch f.sender where (f.receiver = :receiverID and f.sender.id = :senderID) or (f.receiver = :senderID and f.sender.id = :receiverID)")
